...
def get_cart_weight(cart):
    ...
    for item in cart:
        if item.unit in cart_weight:
            ...
            cart_weight[item.per_unit] += float(item.price) / float(item.price_per) * float(
...
def get_cart_total(cart):
    ...
    for item in cart:
        ...
        session["cart_total"] += item.price * session['cart'][str(item.product_id)]
    if session.get('delivery') and session['delivery']['delivery'] == 'delivery'.encode('utf-8'):
